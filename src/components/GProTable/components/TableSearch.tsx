...
const TableSearch = defineComponent({
  ...
  setup(props, { emit, slots }) {
    ...
    const baseClassName = computed(() => `${props.baseClassName}-light-search`)
    ...
    const advancedSlot = ({ formItemStyle, advanced, showAdvanced = true }) =>
      <div style={formItemStyle} class={`${unref(baseClassName)}-form-collapse-button`}>
        <a-space size={16}>
          {optionRender()}
          {showAdvanced && (
    ...
    return () => (
      <div
        class={{
          [`${unref(baseClassName)}`]: true,
          [`${props.className}`]: props.className
        }}
      >
        <a-form class={`${unref(baseClassName)}-form`} layout="horizontal">
          <div class={`${unref(baseClassName)}-form-block`}>
            {formRowSlot()}
          </div>
        </a-form>
      </div>
