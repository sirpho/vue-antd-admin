...
import { isArray, getFileSuffix, getRandomNumber } from '/@/utils/util'
...
export default defineComponent({
  ...
  setup(props, { attrs, emit }) {
    ...
    return () => {
      const { type } = props
      return (
        <ResizeObserver key="resize-observer" onResize={({ width }) => { setResponsive(width < 1540) }}>
          {type && getViewUrl.value && (
            <div class={getClassName.value}>
              <Teleport to="body">
                {showViewer.value && type === '1' && (
                  <w-image-viewer urlList={getViewUrl.value} onClose={() => closeViewer()} />
                )}
              </Teleport>
              <w-modal
                width={850}
