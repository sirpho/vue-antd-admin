      ...
      >
        {new Array(arraySize).fill(null).map((_, index) => (
          ...
          >
            ...
            />
            <w-skeleton active={active} propsStyle={{ height: '48px' }} />
          </div>
        ))}
      </div>
    </a-card>
...
export const ListSkeleton = ({ size, active = true, actionButton }: {
  ...
  <a-card bordered={false} bodyStyle={{ padding: 0 }}>
    ...
    {actionButton !== false && (
      ...
      >
        <w-skeleton active={active} propsStyle={{ width: '102px' }} size="small" />
      </a-card>
    )}
  </a-card>
)
...
export const PageHeaderSkeleton = ({ active }: { active: boolean }) => (
  <div style={{ marginBottom: '16px' }}>
    <a-skeleton
      ...
      paragraph={{ rows: 0, style: { margin: 0 } }}
    />
    <w-skeleton active={active} size="small" />
  </div>
)
...
export const ListToolbarSkeleton = ({ active }: { active: boolean }) => (
  ...
  >
    <a-space style={{ width: '100%', justifyContent: 'space-between' }}>
      <w-skeleton active={active} propsStyle={{ width: '200px' }} size="small" />
      <a-space align="center" direction="horizontal">
        <w-skeleton active={active} propsStyle={{ width: '120px' }} size="small" />
        <w-skeleton active={active} propsStyle={{ width: '80px' }} size="small" />
      </a-space>
    </a-space>
  </a-card>
)
